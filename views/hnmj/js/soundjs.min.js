...
var api=api||Can.api;
var Sound=Sound||{};
Sound.loadAssets = function (d, p, end) {
	if (!d) d = api.data.sounds;
	if (!d) return;
	p = p || "audio/";
	Sound.removeAssets(d, p);
	createjs.Sound.alternateExtensions = ["mp3"];
	createjs.Sound.registerSounds(d, p);
	var n = d.length;

	var a = createjs.Sound.on("fileload", ld, this);

	function ld(e) {
		n--;
		if (n <= 0) {
			createjs.Sound.off("fileload", a);
			if(end)end();
		}
	}
}
Sound.removeAssets = function (d, p) {
	if (!d) d = api.data.sounds;
	if (!d) return;
	p = p || "audio/";
	createjs.Sound.removeSounds(d, p);
}
Sound.play = function (id, l, v = 1, end) {
	if (String(id).match(/\.mp3/)) {
		createjs.Sound.on("fileload", loadHandler);
		createjs.Sound.registerSound(id, id);
	...
	var s = createjs.Sound.play(id, o);
	s.off("complete", a);
	s.volume = v;

	var a = s.on("complete", function () {
		s.off("complete", a);
		if(end)end();

	}, this);
}
Sound.mute = function (id) {
	createjs.Sound.stop();
}